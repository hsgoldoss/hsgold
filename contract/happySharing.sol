...
contract HappySharing is InvestorRelationship, Ownable {

    string private constant FPRS_CURRENCY = "FPRS";
    string private constant TOKEN_CURRENCY = "HSGOLD";
    ...
    function withdrawBalance() public {
        ...
        if (value > 0) {
            require(sero_send_token(msg.sender, FPRS_CURRENCY, value));
        }
    }
    ...
    function invest(string memory refereeCode) public payable returns (bool){
        require(Utils._stringEq(FPRS_CURRENCY, sero_msg_currency()));
        ...
        require(sero_send_token(marketAddr, FPRS_CURRENCY, fee));
        return true;
    }
    ...
    function buyTicket() public payable returns (uint256 asnowAmount) {
        require(isSell);
        require(Utils._stringEq(FPRS_CURRENCY, sero_msg_currency()));
        ...
        if (msg.value > 1e23) {
            amount = 1e23;
            require(sero_send_token(msg.sender, FPRS_CURRENCY, msg.value - amount));
        }
